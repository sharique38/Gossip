...
case class SpawnWork(nodes:Int, topology:String, algo: String,frequency: Int)
case class Rumour(actors:Array[ActorRef], graph:Vector[Vector[Int]], index:Int,frequency:Int)
...
class Node(boss: ActorRef, s: Int, w: Int) extends Actor {
  ...
  def receive = {
    case Rumour(actors: Array[ActorRef],graph:Vector[Vector[Int]], myindex:Int,frequency:Int) =>
      if(rumourCount < maxRumour){
        ...
        if(frequency > 0 && myindex%frequency==0)
        {
          if (!countReached) {
            countReached = true
            boss ! terminate(myindex)
            println("Node" + myindex + " Gossip actor terminated")
            context.stop(self)
          }
        ...
        for(neighbour <- 0 to groupSize) {
          ...
          actors(graph(myindex)(randomNeighbour)) ! Rumour(actors, graph, graph(myindex)(randomNeighbour), frequency)
...
class Boss extends Actor {
  ...
  def receive = {
    case SpawnWork(nodes:Int,topo:String,algorithm: String,frequency:Int) =>
      numofNodes = nodes
      topology = topo
      algo = algorithm
      freq = frequency
      ...
      if("gossip".equalsIgnoreCase(algo)){
        actors(numofNodes/2) ! Rumour(actors, adjList, numofNodes/2,frequency) //trigger
...
object GossipSimulator extends App{
  ...
  if (args.length != 4) {
    ...
    boss ! SpawnWork(nodes, topology.toLowerCase, algo.toLowerCase(),frequency)
  }
